...
/atom/proc/combat_message(var/message,var/blind_message, var/list/targets = list(), var/list/chances = list())
	var/target_message
	target_message = replacetext(message,"'darkredtext","'activefight2")
	target_message = replacetext(target_message,"'observefight","'activefight")
	for(var/mob/M in viewers(src))
		if(targets.Find(M))
			if(chances.Find(ref(M)))
				M.show_message("<span class='ovr'>[target_message]<div class='htxt'>[chances[ref(M)]]</div></span>", 1, blind_message, 2)
			else
				M.show_message(target_message, 1, blind_message, 2)
		else
			M.show_message(message, 1, blind_message, 2)
...
/*		if(/obj/item/surgery_tool/speculum in valid_objects)
			for(var/datum/organ/external/organ in H.organs)
				for(var/obj/item/surgery_tool/speculum/SP in organ.implants)
					if(organ.open != 2)
	...
	if(istype(src,/mob/living/carbon/human))
		...
		if(selection in src.pinned)
			...
			if(!pinned.len)
				anchored = 0
		if(istype(selection, /obj/item/surgery_tool/speculum))
			affected.open = 1
			H.update_surgery(1)
